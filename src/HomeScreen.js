...
const HomeScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    loadSelectedCourseFromAsyncStorage();
  }, []);

  const loadSelectedCourseFromAsyncStorage = async () => {
    try {
  ...
  const saveSelectedCourseToAsyncStorage = async () => {
    try {
      if (selectedCourse) {
  ...
  const handleCourseSelection = (courseName) => {
    if (courseStatuses[courseName]) {
      ...
      saveSelectedCourseToAsyncStorage();
  ...
  const renderCourseButton = ({ item }) => {
    ...
    const courseButtonColor = selectedCourse === courseName ? styles.enabledCourse : (courseStatus ? styles.enabledCourse : styles.disabledCourse);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.courseSection}>
        <View style={styles.courseSubtitle}>
          ...
          <Text style={styles.subtitleText}>Scroll... Choose a course:</Text>
